...
def run_epoch(
    ...
    for batch in loader:
        flows = batch["flow"].to(device, non_blocking=True)
        ...
        if training:
            ...
            optimizer.step()
        total_loss += float(loss.item()) * flows.size(0)
...
def save_confusion_matrix(matrix: list[list[int]] | np.ndarray, classes: list[str], output_path: Path) -> None:
    ...
    ax.set_ylabel("True")
    ax.set_title("CAS(ME)^3 Flow 7-Class Confusion Matrix")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train CAS(ME)^3 7-class motion attention model.")
    ...
    parser.add_argument("--output-dir", type=Path, default=config_get(config, "output_dir", PROJECT_ROOT / "runs" / "casme3_flow_sfamnet"))
    ...
    train_dataset = CASME3FlowDataset(args.manifest, PROJECT_ROOT, augment=True)
    eval_dataset = CASME3FlowDataset(args.manifest, PROJECT_ROOT, augment=False)
    ...
    model = SFAMNetLite(num_classes=len(CASME3_EMOTIONS_7)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history.append(
            {
                "epoch": epoch,
                "train": {key: serializable(value) for key, value in train_metrics.items()},
                "val": {key: serializable(value) for key, value in val_metrics.items()},
            }
        )
